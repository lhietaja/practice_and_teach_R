...
# skippaa tämä jossa teen simulaatiodatan, kuvitteellisessa datassa on kuusi 
# muuttujaa (1a ja 1b esim kuvaamassa eri mittauspisteitä) ja kaksi kategorista
# toinen nimetty jo sukupuoleksi, toinen varmuuden vuoksi
data <- data.frame(
x1a = rpois(n = 100, lambda = 4.5),
x1b = rpois(n = 100, lambda = 3.5),
x2a = rpois(n = 100, lambda = 5.5),
x2b = rpois(n = 100, lambda = 4.2),
x3a = rpois(n = 100, lambda = 2.9),
x3b = rpois(n = 100, lambda = 5.3),
y1 = sample(LETTERS[1:2], 100, replace=TRUE, prob=c(0.7, 0.3)),
y2 = sample(LETTERS[1:2], 100, replace=TRUE, prob=c(0.55, 0.45))
)
data$y1 <- factor(y1, labels = c("Female", "Male"))
data$y2 <- factor(y2, labels = c("Category 1", "Category 2"))
head(data)

write_sav(data, "fakedata.sav")
